...
Lundberg는 **decision trees, random forests and gradient boosted trees**와 같은 트리 기반 기계 학습 모델을 위한 SHAP의 변형인 **TreeSHAP**를 제안했다. Tree SHAP은 빠르고 정확한 Shapley value를 계산하며 특성이 종속인 경우 Shapley Value를 정확하게 추정한다. 이에 비해 KernelSHAP는 계산 비용이 많이 들고 실제 Shapley value만 근사하게 추정한다.
